...
const SETTINGS_DISCLAIMER: &str = "注意：自定义工具权限仅适用于 Zed 原生智能体，不会扩展到通过 Agent Client Protocol (ACP) 连接的外部智能体。";
...
const TOOLS: &[ToolInfo] = &[
    ToolInfo {
        ...
        regex_explanation: "模式会与输入中的每条命令匹配。使用 &&、||、; 或管道连接的命令会拆开后分别检查。",
    },
    ToolInfo {
        id: "edit_file",
        name: "编辑文件",
    ...
    ToolInfo {
        id: "write_file",
        name: "写入文件",
        description: "文件创建和覆盖写入操作",
        regex_explanation: "模式会与正在写入的文件路径匹配。",
    },
    ToolInfo {
        id: "delete_path",
        name: "删除路径",
    ...
    ToolInfo {
        ...
        regex_explanation: "模式会与技能 SKILL.md 文件的绝对路径匹配。",
    },
];
        ...
        .child(
            v_flex()
                ...
                .child(render_rule_section(
                    tool.id,
                    "总是允许",
                    "只要其中任一正则匹配，操作就会被批准，除非同时匹配了“总是确认”或“总是拒绝”。",
                ...
                .child(render_rule_section(
                    tool.id,
                    "总是确认",
                    "只要其中任一正则匹配，就会显示确认提示，除非同时匹配了“总是拒绝”。",
    ...
    v_flex()
        ...
        .children(patterns.iter().map(|pattern| {
            let (type_label, color) = match pattern.rule_type {
                ToolPermissionMode::Deny => ("总是拒绝", Color::Error),
                ToolPermissionMode::Confirm => ("总是确认", Color::Warning),
                ToolPermissionMode::Allow => ("总是允许", Color::Success),
            };
                ...
                if !new_pattern.is_empty() && new_pattern != pattern_for_update {
                    ...
                    let validation_error = if !updated {
                        ...
                        match regex::Regex::new(&new_pattern) {
                            Err(err) => Some(format!(
                                "无效的正则：{err}。模式已保存，但在修复或删除前会阻止此工具。"
                            )),
                            Ok(_) => None,
                        }
                    };
    ...
    h_flex()
        ...
        .child(
            v_flex()
                ...
                .child(
                    Label::new(
                        "控制所有工具操作的默认行为。单个工具的规则和模式可以覆盖此设置。",
                    )
                    .size(LabelSize::Small)
                    .color(Color::Muted),
                ),
